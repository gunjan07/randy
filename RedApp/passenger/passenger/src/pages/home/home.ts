...
export class HomePage {
  ...
  constructor(public nav: NavController, public platform: Platform, public alertCtrl: AlertController,
              ...
              public tripService: TripService, public driverService: DriverService, public authService: AuthService,public chatservice: ChatProvider) {

          this.user = authService.getUserData();


              // if (this.user.mobile && this.user.type) {
        ...
        authService.getUser(this.user.uid).take(1).subscribe(snapshot => {
          this.user = snapshot;
          // console.log(this.driver)
          // if user did not complete registration, redirect to user setting
          if (this.user.email || this.user.v_no || this.user.v_model) {//this.driver.plate &&
            this.loadMap();
          } else {
            this.nav.setRoot(UserPage, {
...
console.log("choise vehicle tyep i"+i);
console.log("choise vehicle tyep index"+index);
  ...
  showDriverOnMap(locality) {
    // get active drivers
    console.log("******************showDriverOnMap "+locality);
    console.log("this.currentVehicle "+ JSON.stringify(this.currentVehicle.name));
    this.driverService.getActiveDriver(locality,this.currentVehicle.name).subscribe(snapshot => {
      console.log(" @@@@@@ "+snapshot);
      console.log(" @@@@@@ "+JSON.stringify(snapshot));

    });
    this.driverService.getActiveDriver(locality, this.currentVehicle.name).take(1).subscribe(snapshot => {
      console.log('fresh vehicles');
console.log(" resuklt "+snapshot);
